            ...
            {filteredDepartments.length > 0 ? (
              <div className="grid grid-cols-3 gap-4">
                {filteredDepartments.map(department => (
                  <ProjectCard key={department.id} id={department.id} path={`department/${department.id}`} name={department.name} members={department.employees ?? []} includeBgImg={true} />
                ))}
              </div>
            ) : (
              <div className="flex flex-col items-center justify-center h-full">
                ...
                <h2 className="text-gray-600 text-lg font-medium mb-1">
                  {searchQuery ? "No matching departments found" : "No Departments at the moment"}
                </h2>
